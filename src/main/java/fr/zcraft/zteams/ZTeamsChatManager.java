...
public class ZTeamsChatManager extends ZLibComponent implements Listener
{
    private final Set<UUID> teamChatLocked = new HashSet<>();
    private final Map<UUID, ZTeam> otherTeamChatLocked = new HashMap<>();
     ...
     */
    public void sendTeamMessage(final Player sender, final String message, final ZTeam team)
    {
        // Permission check
        ...
        if (team == null)
        {
            ...
            recipient = ZTeams.get().getTeamForPlayer(sender);
     ...
     */
    private void sendRawTeamMessage(final String rawMessage, final ZTeam team)
    {
        ...
        // ... and to the console.
        if (ZTeams.settings().teamChatLogInConsole())
        {
     ...
     * @return {@code true} if the chat is now the team chat; false else.
     */
    public boolean toggleChatForPlayer(final UUID playerID, final ZTeam team)
    {
     ...
     * @return {@code true} if the chat is now the team chat; false else.
     */
    public boolean toggleChatForPlayer(final Player player, final ZTeam team)
    {
     ...
     */
    public boolean isTeamChatEnabled(final UUID playerID, final ZTeam team)
    {
        if (team == null)
        {
        ...
        {
            final ZTeam lockedTeam = this.otherTeamChatLocked.get(playerID);
            final ZTeam playerTeam = ZTeams.get().getTeamForPlayer(playerID);
     ...
     * @return {@code true} if the team chat is enabled for the given player.
     */
    public boolean isTeamChatEnabled(final Player player, final ZTeam team)
    {
     ...
     */
    public ZTeam getOtherTeamEnabled(final Player player)
    {
